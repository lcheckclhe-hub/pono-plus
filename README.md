...
src/pages.ts               画面：ログイン / ホーム / シフト入力 / 従業員一覧・登録
test/all.test.ts           単体 + 統合 + ディスパッチャ + 画面 + スキーマ検査（190件）
```
...
```bash
npm run check      # 型チェック + テスト190件（これだけ通せばよい）
npm test
npm run typecheck
```
...
cd pono-plus
npm install
npm run check           # 型チェック + テスト190件。ここが通らないなら先に進まない
```
...
**期待値：20**（テーブル20本）。一致しなければ適用が途中で失敗している。
