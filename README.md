...
| 2. スキーマ適用（テーブル26本・インデックス27本） | ✅ `migrations/` |
...
test/all.test.ts           単体 + 統合 + ディスパッチャ + 画面 + スキーマ検査（395件）
```
...
```bash
npm run check      # 型チェック + テスト395件（これだけ通せばよい）
npm test
npm run typecheck
```
...
cd pono-plus
npm install
npm run check           # 型チェック + テスト395件。ここが通らないなら先に進まない
```
...
**期待値：26**（テーブル26本）。一致しなければ適用が途中で失敗している。
