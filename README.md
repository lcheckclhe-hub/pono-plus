...
| 2. スキーマ適用（テーブル30本・インデックス29本） | ✅ `migrations/` |
...
| 6g. スキルシート（年度別・業務内容の公開制御） | ✅ 画面あり。🔴 本人には残業数と非公開の業務内容を出さない |
| 6h. トップ表示・更新履歴・サポート | ✅ ホームに統合。🔴 動画は埋め込みHTMLではなく動画IDのみ保存 |
...
migrations/0008_skill_sheets.sql              スキルシート（テーブル追加）
migrations/0009_tenant_notices.sql            トップ表示＋サポート（テーブル4本追加）
...
test/all.test.ts           単体 + 統合 + ディスパッチャ + 画面 + スキーマ検査（429件）
```
...
```bash
npm run check      # 型チェック + テスト429件（これだけ通せばよい）
npm test
npm run typecheck
```
...
14. 本人に見せない項目は、返す前に落とす（`redactForEmployee`）。画面側の分岐だけに頼らない
15. 利用者が入力した HTML を保存も出力もしない。動画は ID だけを持ち、埋め込みは自前で組み立てる

**➡ 機能権限表の12区分をすべて実装した**（区分11 ストレスチェックは段階2）。
...
cd pono-plus
npm install
npm run check           # 型チェック + テスト429件。ここが通らないなら先に進まない
```
...
**期待値：30**（テーブル30本）。一致しなければ適用が途中で失敗している。
